...
function [Pbf0,Pcapon0,Pmusic0,PbfC,PcaponC,PmusicC]=SpectralEstimation_AzVel_RT_CLEAN(rc,w_r,lambda,antenna_loc,time_vec,theta,vel,num_tar,mode,c_iter)
...
PbfC    = zeros(length(theta), length(vel));
PcaponC = zeros(length(theta), length(vel));
PmusicC = zeros(length(theta), length(vel));
...
maxIterations = c_iter; loopGain = 0.9; % CLEAN parameters (Controls the optimization jumps)
for cleanMapIterations = 1:maxIterations
    ...
    for tt = 1:num_tar % Three scatterers
        [maxPeakValueIndx_thet(tt), maxPeakValueIndx_vel(tt)] = ind2sub( size(PbfC), sortedLocations_Pbf(tt) );
        
        % -------------------------------------------------------
        % 3. Calculate the CSM induced by the peak source
        
        % Steering vector to location of peak source
        g = 0+URA_az_vel_RT( theta(maxPeakValueIndx_thet(tt)), vel(maxPeakValueIndx_vel(tt)) );
